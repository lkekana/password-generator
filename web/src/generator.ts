...
export const secureRandomInt = createClientOnlyFn(
...
export const secureRandomIntArray = createClientOnlyFn(
	(min: number, max: number, length: number): number[] => {
		const range = max - min + 1;
...
export const generatePassword = createClientOnlyFn(
	(
		length: number,
